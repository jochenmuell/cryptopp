...
public:
    ...
    template <class V, bool A>
    AllocatorWithCleanup(const AllocatorWithCleanup<V, A> &) {}
};
...
private:
    ...
    enum {Q = sizeof(T), PAD = (Q >= 8) ? 1 : (Q >= 4) ? 2
                                          : (Q >= 2)   ? 4
                                                       : 8};
